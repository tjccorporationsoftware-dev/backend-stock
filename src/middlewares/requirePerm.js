...
function requirePermissions(required) {
    return (req, res, next) => {
        ...
        if (!ok) {
            ...
            logActivity(req, `พยายามเข้าถึงข้อมูลที่ไม่มีสิทธิ์ (ต้องการ: ${required.join(',')})`, "Security", userId);
            return res.status(403).json({ message: "คุณไม่มีสิทธิ์เข้าถึงข้อมูลหรือทำรายการในส่วนนี้" });
        }
        next();
    };
}
