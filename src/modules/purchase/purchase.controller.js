...
function resolveSafePdfPath(baseDir, filename) {
  ...
  return {
    safeFilename,
    filePath: path.join(baseDir, safeFilename),
  };
}
...
const createPR = async (req, res, next) => {
  try {
    ...
    if (!items || !Array.isArray(items) || items.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "ต้องระบุสินค้าอย่างน้อย 1 รายการ" });
    }

    if (
      items.some(
        (item) => Number(item.quantity) <= 0 || Number(item.estimatedPrice) < 0
      )
    ) {
      logActivity(
        req,
        "พยายามสร้าง PR ด้วยจำนวนติดลบ/ศูนย์ หรือราคาติดลบ",
        "Security",
        null,
        true
      );
      return res.status(400).json({
        success: false,
        message: "จำนวนและราคาต้องถูกต้องและไม่ติดลบ",
      });
    }
    ...
    res.status(201).json({
      success: true,
      message: "ส่งใบขอซื้อให้ผู้จัดการพิจารณาแล้ว",
      data: pr,
    });
  } catch (error) {
    logger.error("[Purchase] Create PR Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const listPRs = async (req, res, next) => {
  try {
    ...
    if (unused === "true") {
      whereClause.purchaseOrders = { none: {} };
    }
    ...
    logger.error("[Purchase] List PRs Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
    res.status(400).json({
      success: false,
      message: "ไม่สามารถดึงข้อมูลใบขอซื้อได้",
    });
  }
};
...
const getPRDetail = async (req, res, next) => {
  try {
    ...
    if (!pr) {
      ...
      logger.warn(`[Purchase] PR not found: ${safeId}`, {
        ip: req.ip,
        userId: req.user?.id,
      });
    ...
    if (pr.approvals && pr.approvals.length > 0) {
      const latestApproval =
        pr.approvals.find((a) => a.status === "APPROVED") || pr.approvals[0];
    ...
    logger.error("[Purchase] PR Detail Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const approvePR = async (req, res, next) => {
  try {
    ...
    logger.error("[Purchase] Approve PR Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const executeApproveAndIssuePO = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      logActivity(
        req,
        "พยายามสร้าง PO ด้วยจำนวน/ราคาติดลบในขั้นตอนอนุมัติ (Logic Flaw)",
        "Security",
        null,
        true
      );
      return res.status(400).json({
        success: false,
        message: "จำนวนและราคาการสั่งซื้อต้องไม่ติดลบ",
      });
    }

    const [pr, approverData] = await Promise.all([
    ...
    if (!pr || pr.status !== "PENDING") {
      return res
        .status(400)
        .json({ message: "ใบขอซื้อไม่พร้อมสำหรับการอนุมัติ" });
    }
    ...
    logActivity(
      req,
      `อนุมัติ PR และสร้าง PDF PO: ${poNumber}`,
      "Executive_Action",
      finalResult.id
    );
    ...
    res.json({
      success: true,
      pdfUrl: `/api/purchase/po/document/${fileName}`,
    });
  } catch (error) {
    if (browser) await browser.close();
    logger.error("[Purchase] Execute PO Creation Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
// ==========================================
// 6. ระบบเปิดดูเอกสาร PO แบบ Secure
// ==========================================
const viewPODocument = (req, res) => {
  try {
    const { filename } = req.params;
    const resolved = resolveSafePdfPath(UPLOAD_DIR, filename);

    if (!resolved) {
      logActivity(
        req,
        `พยายามเจาะระบบอ่านไฟล์ PO นอกระบบ (Path Traversal): ${filename}`,
        "Security",
        null,
        true
      );
      logger.warn(
        `[Security] Path Traversal Attempt: ${String(filename).replace(/[\r\n]/g, "")}`,
        {
          ip: req.ip,
          userId: req.user?.id,
        }
    ...
    logger.warn(`[Purchase] PO document not found: ${safeFilename}`, {
      ip: req.ip,
      userId: req.user?.id,
    });
    return res.status(404).json({ message: "ไม่พบไฟล์เอกสารบนเซิร์ฟเวอร์" });
  } catch (error) {
    logger.error("[Purchase] View PO Document Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
// ==========================================
// 7. อนุมัติ PR พร้อมสร้าง PDF PR
// ==========================================
const approvePRWithPDF = async (req, res) => {
  ...
  try {
    ...
    if (!fs.existsSync(UPLOAD_DIR_PR)) {
      fs.mkdirSync(UPLOAD_DIR_PR, { recursive: true });
    }
    ...
    const logoHtml = company?.logoUrl
      ? `<img src="${company.logoUrl}" style="max-height: 60px; margin-bottom: 10px;" />`
      : "";
...
await page.pdf({
  ...
  printBackground: true,
    ...
    await prisma.$transaction(async (tx) => {
      await tx.purchaseRequisition.update({
        where: { id: prId },
        data: { status: "APPROVED", pdfPath: filePath },
      });
    ...
    res.json({
      success: true,
      pdfUrl: `/api/purchase/pr/document/${fileName}`,
    });
  } catch (error) {
    if (browser) await browser.close();
    logger.error("[Purchase] Approve PR with PDF Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const createPOFromPR = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      logActivity(
        req,
        "พยายามสร้าง PO จาก PR ด้วยจำนวน/ราคาติดลบ (Logic Flaw)",
        "Security",
        null,
        true
      );
    ...
    if (!pr) {
      return res.status(400).json({ message: "ไม่พบข้อมูลใบขอซื้ออ้างอิง" });
    }

    const procurementFullname = `${procurementUser.firstName} ${procurementUser.lastName}`;
    const prApproval = pr.approvals?.[0];
    const prApproverName = prApproval
      ? `${prApproval.approver.firstName} ${prApproval.approver.lastName}`
      : "ผู้มีอำนาจอนุมัติ";
    const prApproverSignature = prApproval?.signature || null;

    const totalAmount = items.reduce(
      (sum, item) => sum + Number(item.orderedQuantity) * Number(item.unitPrice),
      0
    );
    ...
    res.json({
      success: true,
      message: "สร้างใบสั่งซื้อ PO และ PDF เรียบร้อย",
      pdfUrl: `/api/purchase/po/document/${fileName}`,
    });
  } catch (error) {
    if (browser) await browser.close();
    logger.error("[Purchase] Create PO from PR Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
// ==========================================
// 9. ฟังก์ชันเปิดดูเอกสาร PR แบบ Secure
// ==========================================
const viewPRDocument = (req, res) => {
  try {
    const { filename } = req.params;
    const resolved = resolveSafePdfPath(UPLOAD_DIR_PR, filename);

    if (!resolved) {
      logActivity(
        req,
        `พยายามเจาะระบบอ่านไฟล์ PR นอกระบบ (Path Traversal): ${filename}`,
        "Security",
        null,
        true
      );
    ...
    logger.warn(`[Purchase] PR document not found: ${safeFilename}`, {
      ip: req.ip,
      userId: req.user?.id,
    });
    return res.status(404).json({ message: "ไม่พบไฟล์เอกสาร" });
  } catch (error) {
    logger.error("[Purchase] View PR Document Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
