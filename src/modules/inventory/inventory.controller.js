...
const getStockBalances = async (req, res, next) => {
  try {
    ...
    logger.error("[Inventory] Secure Balances Fetch Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const getStockMovements = async (req, res, next) => {
  try {
    ...
    if (startDate || endDate) {
      ...
      if (endDate)
        whereCondition.createdAt.lte = new Date(`${endDate}T23:59:59Z`);
    }
    ...
    logger.error("[Inventory] Movement Fetch Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const transferStock = async (req, res, next) => {
  try {
    ...
    if (items.some((item) => Number(item.quantity) <= 0)) {
      logActivity(
        req,
        `พยายามโอนย้ายสต๊อกด้วยจำนวนติดลบหรือศูนย์`,
        "Security",
        null,
        true,
      );
      return res
        ...
        .json({
          success: false,
          message: "จำนวนพัสดุต้องเป็นตัวเลขที่มากกว่า 0",
        });
    }
    if (items.some((item) => item.fromLocationId === item.toLocationId)) {
      logActivity(
        req,
        `พยายามโอนย้ายพัสดุเข้าตำแหน่งเดิม (Logic Flaw Attempt)`,
        "Security",
        null,
        true,
      );
      return res
        ...
        .json({
          success: false,
          message: "ตำแหน่งต้นทางและปลายทางห้ามเป็นจุดเดียวกัน",
        });
    }
    ...
    const transferOrder = await prisma.$transaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        if (!fromBalance || fromBalance.quantity < item.quantity) {
          throw new Error(
            `สินค้าในคลัง ${fromBalance?.location.warehouse.name} ไม่พอสำหรับการโอนย้าย`,
          );
        }
    ...
    logActivity(
      req,
      `ทำรายการโอนย้ายสต๊อกสินค้า เลขที่: ${transferNo}`,
      "Inventory",
      transferOrder.id,
    );
    res.json({
      success: true,
      message: `ใบสั่งย้าย ${transferNo} บันทึกสำเร็จ สต๊อกข้ามคลังถูกปรับปรุงแล้ว`,
    });
  } catch (error) {
...
const listTransferOrders = async (req, res, next) => {
  try {
    ...
    logger.error("[Inventory] List Transfer Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const getTransferDetail = async (req, res, next) => {
  try {
    ...
    if (!transferOrder) {
      logger.warn(
        `[Inventory] Transfer not found: ${String(id).replace(/[\r\n]/g, "")}`,
        {
          ip: req.ip,
          userId: req.user?.id,
        }
      ...
      return res
        .status(404)
        .json({ success: false, message: "ไม่พบข้อมูลเอกสารใบโอนย้ายนี้" });
    }

    res.json(transferOrder);
  } catch (error) {
    logger.error("[Inventory] Transfer Detail Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const adjustStock = async (req, res, next) => {
  try {
    ...
    if (!items || !Array.isArray(items) || items.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "กรุณาระบุรายการที่ต้องการปรับปรุง" });
    }
    ...
    if (items.some((item) => Number(item.newQuantity) < 0)) {
      logActivity(
        req,
        `พยายามปรับยอดสต๊อกให้ติดลบ (Logic Flaw Attempt)`,
        "Security",
        null,
        true,
      );
      return res
        .status(400)
        .json({ success: false, message: "ไม่อนุญาตให้ปรับยอดสต๊อกติดลบ" });
    }
    ...
    const adjustment = await prisma.$transaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        if (targetNewQty < 0) {
          throw new Error(
            `ไม่อนุญาตให้ปรับยอดสต๊อกติดลบ (Product ID: ${item.productId})`,
          );
        }
    ...
    logActivity(
      req,
      `ทำรายการปรับปรุงยอดสต๊อกสินค้า เลขที่: ${adjustNo}`,
      "Inventory",
      adjustment.id,
    );
    res.json({
      success: true,
      message: `บันทึกเอกสารปรับปรุงยอด ${adjustNo} สำเร็จ ข้อมูลถูกส่งเข้า Audit Log แล้ว`,
    });
  } catch (error) {
    logger.error("[Inventory] Stock Adjustment Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
    res
      ...
      .json({
        success: false,
        message: error.message || "เกิดข้อผิดพลาดในการปรับปรุงยอด",
      });
  }
};
...
const getAdjustmentDetail = async (req, res, next) => {
  try {
    ...
    if (!adjustment) {
      logger.warn(
        `[Inventory] Adjustment not found: ${String(id).replace(/[\r\n]/g, "")}`,
        {
          ip: req.ip,
          userId: req.user?.id,
        }
      ...
      return res
        .status(404)
        .json({ success: false, message: "ไม่พบข้อมูลเอกสารใบปรับปรุงยอดนี้" });
    }

    res.status(200).json(adjustment);
  } catch (error) {
    logger.error("[Inventory] Adjustment Detail Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const createGoodsReceipt = async (req, res, next) => {
  ...
  try {
    ...
    if (items.some((item) => Number(item.quantity) <= 0)) {
      logActivity(
        req,
        "พยายามรับเข้าสินค้าด้วยจำนวนติดลบหรือศูนย์",
        "Security",
        null,
        true
      );
      return res.status(400).json({
        success: false,
        message: "จำนวนสินค้าที่รับเข้าต้องมากกว่า 0",
      });
    }
    ...
    const receiverName =
      `${receiverUser?.firstName || "System"} ${receiverUser?.lastName || ""}`.trim();
    ...
    const receipt = await prisma.$transaction(async (tx) => {
      if (purchaseOrderId) {
        ...
        if (po.status === "COMPLETED") {
          throw new Error("ใบสั่งซื้อนี้รับสินค้าครบถ้วนและถูกปิดไปแล้ว");
        }

        for (const item of items) {
          const poItem = po.items.find((pi) => pi.productId === item.productId);
          if (!poItem) {
            throw new Error(`สินค้ารหัส ${item.productId} ไม่ได้อยู่ในใบสั่งซื้อนี้`);
          }

          const totalReceivedWillBe =
            poItem.receivedQuantity + Number(item.quantity);

          if (totalReceivedWillBe > poItem.orderedQuantity) {
            throw new Error(
              `ไม่อนุญาตให้รับสินค้าเกินจำนวนที่สั่ง! (สั่ง: ${poItem.orderedQuantity}, รับแล้ว: ${poItem.receivedQuantity}, จะรับเพิ่ม: ${item.quantity})`
            );
          }
    ...
    if (!fs.existsSync(UPLOAD_DIR_GR)) {
      fs.mkdirSync(UPLOAD_DIR_GR, { recursive: true });
    }
    ...
    const securePdfUrl = `/inventory/receipt/document/${fileName}`;
    ...
    const logoHtml = company?.logoUrl
      ? `<img src="${company.logoUrl}" style="max-height: 65px; max-width: 140px; object-fit: contain; margin-bottom: 0 !important;" />`
      : "";
    ...
    const totalQty = receiptData.items.reduce(
    ...
    try {
      ...
      logger.error("[Inventory] Failed to save PDF Path to DB", {
        error: e.message,
      });
    }
    ...
    logActivity(
      req,
      `สร้างใบบันทึกรับสินค้าเข้าคลัง เลขที่: ${receiptNo}`,
      "Inbound",
      receipt.id
    );
    ...
    res.json({
      success: true,
      message: "รับสินค้าเข้าคลังและสร้าง PDF สำเร็จ",
      pdfUrl: securePdfUrl,
    });
  } catch (error) {
    if (browser) await browser.close();
    logger.error("[Inventory] GR Creation Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const getPendingPOs = async (req, res, next) => {
  try {
    ...
    logger.error("[Inventory] Pending POs Fetch Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const getPODetail = async (req, res, next) => {
  try {
    ...
    if (!po) {
      logger.warn(
        `[Inventory] PO not found: ${String(id).replace(/[\r\n]/g, "")}`,
        {
          ip: req.ip,
          userId: req.user?.id,
        },
      ...
      return res
        .status(404)
        .json({ success: false, message: "ไม่พบข้อมูลใบสั่งซื้อ" });
    }
    res.status(200).json(po);
  } catch (error) {
    logger.error("[Inventory] PO Detail Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const createPurchaseOrder = async (req, res, next) => {
  try {
    ...
    if (
      items.some(
        (it) => Number(it.orderedQuantity) <= 0 || Number(it.unitPrice) < 0,
      )
    ) {
      logActivity(
        req,
        `พยายามสร้าง PO ด้วยจำนวนหรือราคาติดลบ/ศูนย์`,
        "Security",
        null,
        true,
      );
      return res
        ...
        .json({
          success: false,
          message: "จำนวนการสั่งซื้อหรือราคาต้องไม่ติดลบหรือเป็นศูนย์",
        });
    }
    ...
    logActivity(
      req,
      `สร้างใบสั่งซื้อใหม่ เลขที่: ${poNumber} จาก PR: ${prId || "Manual"}`,
      "Purchase",
      purchaseOrder.id,
    );
    ...
    res.status(201).json({
      success: true,
      message:
        "สร้างใบสั่งซื้อ (PO) สำเร็จ และได้ทำการปิดยอดใบขอซื้อเรียบร้อยแล้ว",
    });
  } catch (error) {
    logger.error("[Inventory] Create PO Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const getReceiptHistory = async (req, res, next) => {
  try {
    ...
    logger.error("[Inventory] Receipt History Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const getLowStockAlerts = async (req, res) => {
  try {
    ...
    const alerts = products
      .map((p) => {
        const totalQty = (p.stockBalances || []).reduce(
    ...
    logger.error("[Inventory] Low Stock Alert Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const getAllPOs = async (req, res, next) => {
  try {
    ...
    logger.error("[Inventory] Get All POs Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const generatePOPdf = async (req, res, next) => {
  ...
  try {
    ...
    const subTotal = po.items.reduce(
      (sum, item) =>
        sum + Number(item.orderedQuantity) * Number(item.unitPrice),
      0,
    );
    ...
    logger.error("[Inventory] Premium PDF Generation Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
...
const viewGRDocument = (req, res) => {
    try {
        ...
        if (!isValidPdfName) {
            ...
            logger.warn(`[Inventory] Path Traversal Attempt: ${String(filename).replace(/[\r\n]/g, '')}`, {
                ip: req.ip,
                userId: req.user?.id
            });
        ...
        const filePath = path.join(process.cwd(), 'public', 'uploads', 'grs', safeFilename);
        ...
        logger.error("[Inventory] View GR Document Error", {
            error: error.message,
            ip: req.ip,
            userId: req.user?.id
        });
        return res.status(500).json({ message: "ระบบประมวลผลไฟล์ล้มเหลว" });
    }
};
...
const getAgedStock = async (req, res, next) => {
  try {
    ...
    logger.error("[Inventory] Aged Stock Error", {
      error: error.message,
      ip: req.ip,
      userId: req.user?.id,
    });
